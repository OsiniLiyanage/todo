 ...
 * @author acer
 */
@WebServlet(name = "ToggleDone", urlPatterns = {"/ToggleDone"})
public class ToggleDone extends HttpServlet {
    ...
    protected void doPut(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (todoIdStr == null || userIdStr == null) {
                ...
                if (todo == null) {
                    ...
                    if (user == null || !todo.getUser().equals(user)) {
                        ...
                        boolean currentStatus = todo.isIsDone();
                        transaction = session.beginTransaction();
                        todo.setIsDone(!currentStatus);
                        session.update(todo);
                        transaction.commit();

                        responseObject.addProperty("status", true);
                        responseObject.addProperty("message", "Task status updated");
                    }
                }
            }
        } catch (Exception e) {
            ...
            e.printStackTrace();
            responseObject.addProperty("message", "Failed to update task status");
        } finally {
            session.close();
        }
